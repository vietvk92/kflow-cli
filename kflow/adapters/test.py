...
class TestAdapter(DetectionAdapter):
    """Scaffold for configured test command detection."""
    ...
    def execute(self, repo_root: Path) -> ShellResult:
        if not self.enabled:
            return ShellResult(command=["test"], returncode=1, stdout="", stderr="test adapter disabled")
        if not self.command:
            return ShellResult(command=["test"], returncode=1, stdout="", stderr="test command not configured")
